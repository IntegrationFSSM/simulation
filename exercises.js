...
const ExerciseStorage = {
    ...
    getEntryCount(patientId, exerciseId) {
        const all = this._loadAll();
        const prefix = `p${patientId}_${exerciseId}_`;
    ...
    clearExercise(patientId, exerciseId) {
        const all = this._loadAll();
        const prefix = `p${patientId}_${exerciseId}`;
        Object.keys(all).filter(k => k === prefix || k.startsWith(prefix + '_')).forEach(k => delete all[k]);
        this._saveAll();
    }
};
